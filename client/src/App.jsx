...
function App() {
  ...
  const filteredOutbreaks = outbreaks.filter(
    (o) => !selectedDate || o.reportedDate?.slice(0, 10) === selectedDate,
  );

  async function calculateTravel() {
    console.log(
      "calculateTravel called",
      targetCity,
      selectedOutbreak?.latitude,
    );
    ...
    const a =
      Math.sin(dLat / 2) ** 2 +
      Math.cos((selectedOutbreak.latitude * Math.PI) / 180) *
          ...
          {selectedOutbreak ? (
            ...
            >
              <p style={{ fontSize: "12px", margin: "0" }}>
                From: <b>{selectedOutbreak.city || selectedOutbreak.disease}</b>
              </p>
              <input
                type="text"
            ...
            <p style={{ fontSize: "12px", color: "var(--text)" }}>
              Select an outbreak from the table to calculate travel risk.
            </p>
          )}
        </div>
      </div>
  ...
  return (
    <div className="app-container">
      ...
      <footer className="footer">
        Developed by Olukolajo Sodipe (Intern) Inayat Chandra (Intern) & Anuj
        Tiwari (Supervisor) · DPI{" "}
      </footer>
    </div>
  );
}
